...
async function loadChatMessages() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Start polling only when chat is open (optimized intervals)
            if (!chatPollingInterval && isChatOpen) {
                chatPollingInterval = setInterval(pollNewMessages, 8000);
            }
            
            // Start typing poll only when needed
            if (!typingPollingInterval && isChatOpen) {
                typingPollingInterval = setInterval(pollTypingUsers, 2000);
            }
        }
    } catch (error) {
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            lastChatTimestamp = data.message.createdAt;
...
async function toggleReaction(msgId, emoji) {
    try {
        ...
        if (response.ok) {
            ...
            if (msgIndex >= 0) {
                chatMessages[msgIndex].reactions = data.reactions;
                renderChatMessages();
            }
        } else {
            const error = await response.json();
